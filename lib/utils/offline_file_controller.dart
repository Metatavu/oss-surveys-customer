...
class OfflineFileController {
  ...
  Future<File?> getOfflineFile(String url, { download = true }) async {
    try {
      if (download) {
        ...
        File offlinedFile = File(p.join((await _getImagesDirectoryPath()), _getOfflineFileName(url)));
  ...
  Future<File?> _download(String url) async {
    ...
    File? metaFile = await _getDirectoryFileByNameAndExt(await _getImagesDirectoryPath(), fileName, ".meta");
    ...
    switch (response.statusCode) {
      ...
      case 304: {
        ...
        return await _getDirectoryFileByNameAndExt(await _getImagesDirectoryPath(), fileName, ".jpeg");
      }
      default: {
  ...
  Future<String> _getMetaFilePath(File file) async => "${(await _getImagesDirectoryPath())}/${p.withoutExtension(p.basename(file.path))}.meta";

  /// Handles creating of new file with [fileName].
  /// 
  /// ETag and file content is parsed from [response].
  Future<File?> _handleNewFile(String fileName, HttpClientResponse response) async {
  ...
  Future<Int8Buffer> readResponseToBytes(HttpClientResponse response) {
    ...
    response.listen((data) => byteBuffer.addAll(data), onDone: () => completer.complete(byteBuffer));
  ...
  Future<String> _getImagesDirectoryPath() async {
    return (await Directory("${(await getApplicationSupportDirectory()).path}/images").create()).path;
  }
  ...
  Future<File?> _getDirectoryFileByNameAndExt(String directoryPath, String fileName, String fileExt) async {
    try {
      return File((await Directory(directoryPath)
        .list()
        .firstWhere((fileSystemEntity) => p.basenameWithoutExtension(fileSystemEntity.path) == fileName && p.extension(fileSystemEntity.path) == fileExt)).path
    );
    } catch (e) {
...
class MetaFile {
  ...
  factory MetaFile.fromJson(Map<String, dynamic> json) => MetaFile(json["eTag"] as String?);
